...
export default function CameraScreen() {
  ...
  const output = usePhotoOutput({ targetResolution: PHOTO_RESOLUTION, containerFormat: 'jpeg', quality: 1, qualityPrioritization: 'speed' });
  const focused = useIsFocused(); const appActive = AppState.currentState === 'active'; const { height } = useWindowDimensions();
  const [preview, setPreview] = useState<Size>({ width: 0, height: 0 }); const [sessionStarted, setSessionStarted] = useState(false); const previewHeight = Math.max(320, Math.min(520, height * 0.56));
  ...
  const recognition = useProductRecognition(products, modelInfo, output, preview, roi, ready && focused && appActive && sessionStarted);
  ...
  return <SafeAreaView style={styles.safe}><View style={styles.header}><Text style={styles.title}>Câmera</Text><Text style={styles.subtitle}>Reconhecimento 100% offline</Text></View>
    <View style={[styles.preview, { height: previewHeight }]} onLayout={(event) => setPreview(event.nativeEvent.layout)}>
      <Camera style={StyleSheet.absoluteFill} device={device} outputs={[output]} isActive={focused && appActive} resizeMode="cover" orientationSource="device" onStarted={() => setSessionStarted(true)} onStopped={() => setSessionStarted(false)} onError={(error) => console.warn(error)} />
